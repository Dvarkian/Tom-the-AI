import sys
import os
import importlib

dir_ = os.getcwd() + "/" # Find directory from which program is running.
sys.path.insert(0, dir_ + "generic_modules") # Modules used by both windows and linux systems.

import PySimpleGUI as sg


avgWordChars = 5.1 # [chars]
kernedFont = "Helevicta"


activeResponses = os.listdir(dir_ + "responses")
archiveResponses = os.listdir(dir_ + "response_archive")

order = open("responseOrder.txt", "r").readlines()

sortedModules = []

#print(len(order), order)

for r in order:
    hold = r
    r = str(r.replace("\\n", "").replace("\\n", "")).strip("\n")

    #print(r, "::::", archiveResponses)
    
    if r in activeResponses:
        sortedModules.append(r)
        #print("foundA")
    elif r in archiveResponses:
        #print("Found")
        sortedModules.append(r)
        archiveResponses.remove(r)
        os.rename(dir_ + "response_archive/" + r, dir_ + "responses/" + r)
    else:
        order.remove(hold)

#print("S:", sortedModules)

for r in activeResponses:
    if r not in sortedModules:
        os.rename(dir_ + "responses/" + r, dir_ + "response_archive/" + r)

activeResponses = os.listdir(dir_ + "responses")
archiveResponses = os.listdir(dir_ + "response_archive")


def findIndex(list_, item):
    for i in range(0, len(list_)):
        if list_[i] == item:
            return i
    else:
        return False
    

def loadWin(sortedModules):
    print(sortedModules)
    
    fontSize = 12
    rowBg = "purple"
    
    col = [[sg.Text("Active Responses:", font=(kernedFont, fontSize), text_color="orchid3",
                           background_color="grey20")]]

    file = open("responseOrder.txt", "w")
    file.close()

    file = open("responseOrder.txt", "a")

    for response in sortedModules:

        file.write(str(response) + "\n")

        try:
            with open(dir_ + "responses/" + response + "/info.txt") as infoFile:
                info = eval(infoFile.read())
                descr = info["descr"]
        except FileNotFoundError:
            descr = ""
            
        col.append([sg.Col(layout=[[sg.Text(" " + response.capitalize().replace("_", " "), size=(28, None),
                           font=(kernedFont, fontSize), text_color="light blue",
                           background_color="grey10", pad=((3, 3), (2, 0))),
                    sg.Button(k="-UP-"+response, font=(kernedFont, 5), image_filename="graphics/up.png",
                              button_color=("light blue", rowBg), image_subsample=2,
                              border_width=0, pad=((0, 0), (0, 0))),
                    sg.Button(k="-DOWN-"+response, font=(kernedFont, 5), image_filename="graphics/down.png",
                              button_color=("light blue", rowBg), image_subsample=2,
                              border_width=0, pad=((1, 4), (0, 0)))],
                    [sg.Text(descr, font=(kernedFont, 7), text_color="grey99", background_color=rowBg,
                             pad=((3, 0), (0, 0)), k="-DESCR-"+response)]],
                           background_color=rowBg)])

    file.close()

    if len(archiveResponses):
        col.append([sg.HorizontalSeparator(color="dark blue")])
        col.append([sg.Text("Disabled Responses:", font=(kernedFont, fontSize), text_color="light blue",
                             background_color="grey20")])

        for response in archiveResponses:
            
            try:
                with open(dir_ + "response_archive/" + response + "/info.txt") as infoFile:
                    info = eval(infoFile.read())
                    descr = info["descr"]
            except FileNotFoundError:
                descr = ""
                
            col.append([sg.Col(layout=[[sg.Text(response.capitalize().replace("_", " "), size=(28, None),
                               font=(kernedFont, fontSize), text_color="grey60",
                               background_color="grey10", pad=((3, 3), (2, 0))),
                        sg.Button(k="-UP-"+response, font=(kernedFont, 5), image_filename="graphics/up.png",
                               button_color=("light blue", "grey30"), image_subsample=2,
                               border_width=0, pad=((0, 4), (0, 0)))],
                        #sg.Button(k="-DOWN-"+response, font=(kernedFont, 5), image_filename="graphics/down.png",
                        #       button_color=("light blue", "grey30"), image_subsample=2,
                        #       border_width=0, pad=((1, 4), (0, 0)))
                        [sg.Text(descr, font=(kernedFont, 7), text_color="grey5", background_color="grey30",
                             pad=((3, 0), (0, 0)))]],
                               background_color="grey30")])
    # Graphics for toggle buttons.
    toggle_btn_off = b''
    toggle_btn_on = b''

    modulesBgCol = "grey20"

    # Define layout of interface modules.

    colWidth = 360
    colHeight = 450

    # Define layout of modules window.
    
    modulesLayout = [[sg.Text("Tom's responses can be modified below ...", background_color="grey30",
                               font=(kernedFont, 12))],
                      [sg.Column(background_color=modulesBgCol, layout=col,
                                 size=(colWidth, colHeight), element_justification="center",
                                 scrollable=True, vertical_scroll_only=True, key="-COL-")],
                      [sg.Button("Done", key="-DONE-", font=(kernedFont, 12), button_color=("white", "grey15")),
                       sg.Button("Cancel", key="-CANCEL-", font=(kernedFont, 12), button_color=("red", "grey15"))]]


    # Define arameters of modules window.

    modulesWindow = sg.Window('Tom | Modules', modulesLayout,
        grab_anywhere=False,
        keep_on_top=True,
        background_color="grey30",
        alpha_channel=.98,
        finalize=True,
        icon="graphics/squareFace.png",
        margins=(15, 15))


    return modulesWindow

    
def moduleBrowser():
    global sortedModules

    modulesWindow = loadWin(sortedModules)

    while 1: # Event loop for modules window.
        
        event, values = modulesWindow.read() # Read events from window.

        if event == sg.WIN_CLOSED or event == "-CANCEL-":
            modulesWindow.close()
            break

        elif event == "-DONE-":
            modulesWindow.close()
            break
            #quit()

        elif "-UP-" in event:
            item = event.split("-UP-")[1]
            if item in sortedModules:
                index = findIndex(sortedModules, item)
                if index > 0:
                    sortedModules[index] = sortedModules[index-1]
                    sortedModules[index-1] = item
                    modulesWindowNew = loadWin(sortedModules)
                    modulesWindow.close()
                    modulesWindow = modulesWindowNew
            else:
                sortedModules.append(item)
                os.rename(dir_ + "response_archive/" + item, dir_ + "responses/" + item)
                archiveResponses.remove(item)
                modulesWindowNew = loadWin(sortedModules)
                modulesWindow.close()
                modulesWindow = modulesWindowNew
                

        elif "-DOWN-" in event:
            item = event.split("-DOWN-")[1]
            index = findIndex(sortedModules, item)
            if index < len(sortedModules)-1:
                sortedModules[index] = sortedModules[index+1]
                sortedModules[index+1] = item
                modulesWindowNew = loadWin(sortedModules)
                modulesWindow.close()
                modulesWindow = modulesWindowNew
            else:
                sortedModules.remove(item)
                os.rename(dir_ + "responses/" + item, dir_ + "response_archive/" + item)
                archiveResponses.append(item)
                modulesWindowNew = loadWin(sortedModules)
                modulesWindow.close()
                modulesWindow = modulesWindowNew


if __name__ == "__main__":
    moduleBrowser()

